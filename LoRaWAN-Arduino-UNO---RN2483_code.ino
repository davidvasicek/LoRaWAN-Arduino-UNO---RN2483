...
// constant
int8_t LED_STATUS = 6; // externi LED
int8_t LED_RESIST = 9; // integrovana LED
...
void setup()
{
  ...
  pinMode(4, OUTPUT); // output reset rn2483 pin
    
  pinMode(LED_STATUS, OUTPUT); //output LED pin
  pinMode(LED_RESIST, OUTPUT); //output LED pin
...
void loop()
{
    ...
    myLora.tx("text"); //one byte, blocking function

    led_off();

    if(i==6){

      //initialize_radio(); // reset rn2483 - vymaze citac pro duty cycle

      i=0;
      
    }

    delay(120000); // interval, ve kterem se zpravy odesilaji 120 sec
}
